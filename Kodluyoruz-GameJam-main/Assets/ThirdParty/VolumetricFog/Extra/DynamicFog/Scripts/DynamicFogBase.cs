...
namespace DynamicFogAndMist
{
    ...
    public class DynamicFogBase : MonoBehaviour
    {
        ...
        [SerializeField]
        protected bool _useSinglePassStereoRenderingMatrix = false;
            ...
            switch (effectType)
            {
                case FOG_TYPE.MobileFogOnlyGround:
                    if (fogMatOnlyFog == null)
                    {
                        ...
                        fogMatOnlyFog = Instantiate(Resources.Load<Material>(matName));
        ...
        public void SetFogOfWarAlpha(Vector3 worldPosition, float radius, float fogNewAlpha)
        {
            ...
            if (newAlpha8 != existingColor.a)
            { // just to avoid over setting the texture in an Update() loop
